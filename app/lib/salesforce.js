...
function Salesforce(params) {
	...
	this.headers = [
		{
		name: "Authorization",
		value: "OAuth " + this.accessToken
	},
		{
		name: "X-User-Agent",
		value: "salesforce-toolkit-rest-javascript/" + this.apiVersion
	},
		{
		name: "Content-Type",
	...
	this.getAuthorizeUrl = function(loginUrl, clientId, redirectUri) {
		return this.loginUrl + 'services/oauth2/authorize?display=touch' + '&response_type=token&client_id=' + escape(this.clientId) + '&redirect_uri=' + escape(this.redirectUri);
	};
		...
		http.request({
			...
			success: function(request) {
				...
				if(request.id) {
					...
					Ti.App.Properties.setString('currentUser', that.currentUser);
				...
				that.headers = [
	           		{
					name: "Authorization",
					value: "OAuth " + that.accessToken
				},
	           		{
					name: "X-User-Agent",
					value: "salesforce-toolkit-rest-javascript/" + that.apiVersion
				},
		            {
					name: "Content-Type",
					value: "application/json"
	...
	this.login = function(callback) {
		...
		var webView = Ti.UI.createWebView({
			url: this.getAuthorizeUrl(this.loginUrl, this.clientId, this.redirectUri)
		});
		newWin.add(webView);
		newWin.open();
