	...
	if( dataCount > 0 ){
		for( i=0; i<dataCount; i++){
			if( userid === data[i].userID ){ //data[i]["[ACS_User]user_id"][0].id
				outletData = data[i];
				var outlet = outletData["[CUSTOM_outlets]outlet_id"] || outletData["[custom_outlets]outlet_id"];
				...
				outletData = {
						...
						outlet: {
							name: outlet[0].name,
							address: outlet[0].address_1,
							city: outlet[0].City,
							state: outlet[0].State,
							zip: outlet[0].zip,
							latitude: outlet[0].latitude,
							longitude: outlet[0].longitude,
							contactName: outlet[0].contactName,
							contactPhone: outlet[0].contactPhone,
							contactEmail: outlet[0].contactEmail,
							oid: outlet[0].oid,
							aid: outletData.aid,
							id: outlet[0].id
						}					
...
var _transformMulti = function(type, data, _cb){
	...
	var outlet = data["[CUSTOM_outlets]outlet_id"] || data["[custom_outlets]outlet_id"];
	switch(type){
	case 'assignment':
				results = {
					status: data.status,
	...
	break;
	case 'outlet':
				var customData = data[0]["[CUSTOM_outlets]outlet_id"] || data[0]["[custom_outlets]outlet_id"];
				results = {
					...
					outlet: {
						...
						address: customData[0].address_1,
						city: customData[0].City,
						state: customData[0].State,
						zip: customData[0].zip,
						latitude: customData[0].latitude,
						longitude: customData[0].longitude,
						contactName: customData[0].contactName,
						contactPhone: customData[0].contactPhone,
						contactEmail: customData[0].contactEmail,
						oid: customData[0].oid,
						id: customData[0].id
					}
				};		
	break;
	...
	case 'products':
		...
		if( data.success && data.meta.total_results > 0 ){
			...
			for(var i=0;i<data.products.length;i++){
				var catalogData = data.products[i]["[CUSTOM_catalog]catalog_id"] || data.products[i]["[custom_catalog]catalog_id"];
				results.push({
					id: data.products[i].id,
					aid: data.products[i].aid,
					cid: data.products[i].cid,
					quantity: data.products[i].quantity,
					productName: (catalogData && catalogData[0]) ?catalogData[0].name:'No Product Name',
					productDescription: (catalogData && catalogData[0]) ? catalogData[0].description: 'No Product Description',
					productStatus: (catalogData && catalogData[0]) ?catalogData[0].status: 'No Product Status',
					productCID: (catalogData && catalogData[0]) ?catalogData[0].cid: 'No Product Catalog ID'
