...
var _transform = function(data,_cb){
	...
	if( dataCount > 0 ){
		for( i=0; i<dataCount; i++){
			  	var customData = data[i]["[CUSTOM_catalog]catalog_id"] || data[i]["[custom_catalog]catalog_id"];
				results.push({
					id: data[i].id,
					aid: data[i].aid,
					cid: data[i].cid,
					quantity: data[i].quantity,
					productName: (customData && customData[0]) ? customData[0].name: 'No Product Name',
					productDescription: (customData && customData[0]) ?customData[0].description:'No Product Description',
					productStatus:(customData && customData[0]) ? customData[0].status: 'No Product Status',
					productCID: (customData && customData[0]) ?customData[0].cid : 'No Product Catalog ID'
